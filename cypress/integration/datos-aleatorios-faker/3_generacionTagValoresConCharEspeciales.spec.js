...
context('Actions', () => {
    ...
    it('Fill form tag info', function (){
      cy.screenshot() 
      cy.get('#tag-name').type(faker.internet.email(),{force: true})
      cy.get('#tag-slug').type(faker.internet.email(),{force: true})
      cy.get('#tag-slug').clear({force: true})
      cy.get('#tag-slug').type(faker.internet.email(),{force: true})
      cy.get('#tag-description').type(faker.lorem.paragraphs(2),{force: true})
